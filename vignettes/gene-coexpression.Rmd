...
I next wanted to find the correlation of each gene compared against the others across mice. For that, I used the `cor.test` function. I initiated `cov_tib` with two columns containing each gene-to-gene comparison.
...
To limit the number of false positives, I used the Benjamini-Hochberg FDR corrected p-value (a.k.a. q-value), then only kept the genes with a value below $0.05$, deeming those as "significantly" correlated. I also only kept the genes with Pearson $\rho$ correlation values above $0.7$ or below $-0.7$ (this is an arbitrary cut-off to narrow down the genes being inspect).
...
Finally, I can plot the results in a symmetric matrix using `geom_asymmat`. I will use the $-log_{10}(\text{q-value})$ and Pearson $\rho$ to fill the top-left and bottom-right triangle, respectively.

```{r plot}
asym_cov_tib <- asymmetrise(cov_tib, gene_a, gene_b)
ggplot(asym_cov_tib, aes(x = gene_a, y = gene_b)) +
