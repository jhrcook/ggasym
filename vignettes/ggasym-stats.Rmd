...
One of the great uses of ggasym is to plot two values from the results of a multi-way statistical test. Each comparison is a cell, an two values can be used for the fill. Below I give breif examples and plot the differences in mean and the p-value on the symmetric matrix.
...
And add a few styling changes with normal ggplot2 symantics.
...
One of the conlusions that can be drawn here is that the difference in expression of kindlin and FAK is the greatest and has a very low adjusted p-value. Thus, one of the conclusion is that kindlin is expressed at a lower level than FAK.
